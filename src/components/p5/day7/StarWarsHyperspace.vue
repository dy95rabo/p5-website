...
const sketch = (p5) => {
  ...
  class Star {
    ...
    static maxZ = 800;
    static maxRadius = 30;
    static startingArea = 0.8;
    static starDensityPer1000Pixel = 4;
    static stars = [];
    ...
    draw() {
      p5.fill(this.color);
      // I don't really understand this part of the code, but it seems to work
    ...
    randomPos() {
      this.startingX = p5.random(
        -p5.width * Star.startingArea,
        p5.width * Star.startingArea
      );
      this.startingY = p5.random(
        -p5.height * Star.startingArea,
        p5.height * Star.startingArea
      );
    }
    ...
    static init() {
      let relativeScreenSize = p5.createVector(p5.width, p5.height).mag()/p5.createVector(1920, 1039).mag()
      Star.maxRadius *= relativeScreenSize
      //not sure how to fix starting area for small screens
      // Star.startingArea = relativeScreenSize/Star.startingArea
        
      Star.numberOfStars = Math.floor(
        (p5.width * p5.height) / (Star.starDensityPer1000Pixel * 1000)
      );
...
const keyInput = [
  ...
  {
    keys: "'click'",
    function: "accelerate/decelerate spaceship",
  },
];
</script>
